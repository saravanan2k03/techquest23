...
class _DetailState extends State<Detail> {
  ...
  void writeFie(String data) async {
    try {
      await File('C:/CanteenData/entries.txt').create(recursive: true);
      File file = File('C:/CanteenData/entries.txt');
  ...
  Future Addref(String teamid) async {
    ...
    writeFie(data.toString());
    try {
      final response = await http.post(
        Uri.parse('http://103.207.1.94:8080/addstudent'),
        body: {
          'user': student.join(';'),
          'qr': qrcode.join(';'),
          "teamid": teamid,
      ...
      if (response.statusCode == 200) {
        ...
        customshowAlertDialog("Warning", "QrCode Not Found!");
