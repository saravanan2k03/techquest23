...
class _LoginPageState extends State<LoginPage> {
  ...
  void LoginFunction() {
    if (user.text.isEmpty || pwd.text.isEmpty) {
      customshowAlertDialog("Warning", "Please Enter UserName or Password!");
    } else {
      if (user.text == "admin" && pwd.text == "Mount@1234") {
        box.write('usertype', 'admin').whenComplete(() {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => const HomeScreen()),
          );
        });
      } else if (user.text == "codinator" && pwd.text == "Techquest@2023") {
        box.write('usertype', 'co').whenComplete(() {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const ArrivalPage()),
          );
        });
      } else {
        customshowAlertDialog(
            "Warning", "Please Make Sure Your UserName or Password!");
      }
    }
  }
